...
export function Link({ to, href, ...props }) {
  ...
  return <NextLink to={url} className={styles.link} {...props} />;
}
...
export function Avatar({ alt, image }) {
  return <Image alt={alt} image={getImage(image)} className={styles.avatar} />;
}

export function Logo({ alt, image, size = 'small' }) {
  return (
    <Image alt={alt} image={getImage(image)} className={styles.logos[size]} />
  );
}

export function Icon({ alt, image, size = 'medium' }) {
  return <img src='/public/TWILogo.png' alt='logo' />;
}
